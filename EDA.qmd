...
diabetes_summary_long <- diabetes_summary |>
  pivot_longer(
    cols = -c(Diabetes_binary, n, pct_total), 
    names_to = "metric",
    values_to = "value"
  ) |>
  mutate(
    metric = str_remove_all(metric, "pct_|mean_"),
    metric = str_to_title(str_replace_all(metric, "_", " "))
  )|>
  select(metric, everything())

knitr::kable(diabetes_summary_long, 
             digits = 1,
             col.names = c("Metric","Diabetes Status", "Count", "% of Total", "Value"))
...
# Contingency table heat map for categorical variables
